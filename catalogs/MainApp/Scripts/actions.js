...
Actions.vote = function(params) {
    Actions.account.vote(params["author"], params["permlink"], parseInt(params["weight"]), function(response) {
        if (response) {
            Actions.__get_content(params["author"], params["permlink"], function(data) {
                ...
                Actions.__on_complete(params, data);
            });
        } else {
...
Actions.comment = function(params) {
    Actions.account.comment(params["parent-author"], 
        ...
        if (response) {
            Actions.__get_content(params["parent-author"], params["parent-permlink"], function(data) {
...
Actions.follow_user = function(params) {
    Actions.account.follow_user(params["username"], function(response) {
        if (response) {
            Actions.__get_user(params["username"], function(data) {
...
Actions.transfer = function(params) {
    Actions.wallet.transfer(params["to"], 
        ...
        if (response) {
            Actions.__get_user_simple(Actions.account.get_username(), function(data) {
                Actions.__on_complete(params, data);
            });
        }
    });
...
Actions.__get_content = function(author, permlink, handler) {
    Actions.steemjs.get_content(author, permlink).then(function(response) {
        if (response) {
            ...
            var identifier = "content-" + content.data["author"] + "." + content.data["permlink"];
            var data = {
            ...
            controller.update(identifier, data);
            handler(Object.assign(data, { "data-binding":identifier }));
        } else {
            handler();
        }
    }, function(reason) {
...
Actions.__get_user_simple = function(username, handler) {
    Promise.all([
        Actions.steemjs.get_accounts([ username ]),
        Actions.steemjs.get_follow_count(username),
        Actions.steemjs.get_dynamic_global_properties()
...
Actions.__on_complete = function(params, data) {
