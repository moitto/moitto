...
function feed_blog(keyword, location, length, sortkey, sortorder, handler) {
    ...
    __get_discussions_by_created($data["tag"], start_author, start_permlink, length, [], function(discussions) {
...
function __get_discussions_by_created(tag, start_author, start_permlink, length, discussions, handler) {
    steemjs.get_discussions_by_created(tag, start_author, start_permlink, length + (start_author ? 1 : 0)).then(function(response) {
        ...
        if (discussions.length < length && response.length > 0) {
            ...
            __get_discussions_by_created(tag, start_author, start_permlink, length, discussions, handler);
